...
public class FileMgr {
   ...
   private int blocksRead = 0;  // New variable to keep track of blocks read
   private int blocksWritten = 0;  // New variable to keep track of blocks written
   ...
   synchronized void read(Block blk, ByteBuffer bb) {
      try {
         ...
         blocksRead++;  // Increment the blocksRead counter
      }
      catch (IOException e) {
   ...
   synchronized void write(Block blk, ByteBuffer bb) {
      try {
         ...
         blocksWritten++;  // Increment the blocksWritten counter
      }
      catch (IOException e) {
